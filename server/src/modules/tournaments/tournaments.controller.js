...
export async function createCallups(req, res, next) {
  try {
    const data = await service.createCallups(req.context.activeAcademyId, req.params.id, req.body.athlete_enrollment_ids, req.context.profileId);
    return created(res, data);
  } catch (err) { next(err); }
}
...
export async function launchCallups(req, res, next) {
  try {
    const data = await service.launchCallups(req.context.activeAcademyId, req.params.id, req.context.profileId);
    return success(res, data);
  } catch (err) { next(err); }
}
